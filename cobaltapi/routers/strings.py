...
@STRINGS_ROUTER.get('/upper')
@STRINGS_ROUTER.get('/uppercase')
...
@STRINGS_ROUTER.get('/lower')
@STRINGS_ROUTER.get('/lowercase')
async def uppercase(string: str):
    """
...
async def similarity(original: str, edited: str):
    ...
    return {
        "original": original,
        "edited": edited,
        "similarity": SequenceMatcher(None, original, edited).ratio() * 100
    }



@STRINGS_ROUTER.get("/replace")
async def replace(string: str, find: str, replace_with: str):
    ...
    return {
        "original": string,
        "new": string.replace(find, replace_with)
    }



@STRINGS_ROUTER.get("/split")
async def split(string: str, separator: str, max_split: int = -1, strip: bool = True):
    ...
    return {
        "original": string,
        "split": string.split(separator, max_split) if not strip
